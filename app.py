...
"""
Hush App – Backend Logic

This file handles all backend logic for the Hush app. It can take either:
- a video URL (downloaded using yt-dlp), or
- a video file uploaded by the user.

After receiving the video, the backend sends it to the MediaProcessor C++ program,
which removes noise and creates a new processed video file.

Finally, the backend returns a JSON response that includes the URL of the processed
video so the user can watch or download it from the frontend.
"""
